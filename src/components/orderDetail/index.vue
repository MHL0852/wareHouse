...
export default {
  ...
  methods: {
    ...
    initMap(){
      ...
      // const unikey = 'e8d9e9324af6b968b2322c2bd2baeb0e'
      // const id = '762'
      // const unikey = '528d0a1b2c5e5dd54d51d0c4ec59a09e'
      // const id = '765'
      ...
      // 获取起始点
      utilFos('/OrderMapServlet?action=getStartEndPoint', {
        params: {
        ...
        if(response.status == 200){
          // 发货仓
          var fromPoint = new BMap.Point(result.frontWarehouse.lng, result.frontWarehouse.lat);
          var fromMarker = new BMap.Marker(fromPoint, {icon: result.frontWarehouse.type == '1' ? wareIcon : preWareIcon});
          var fromName = result.frontWarehouse.type == '1' ? '总仓：' : '前置仓：';
          var fromLabel = new BMap.Label('<div class="ware_detal"><p>'+ fromName + result.frontWarehouse.name +'</p></div>', { offset: new BMap.Size(34,-10) });
          ...
          // 收货点
          var toPoint = new BMap.Point(result.receiver.lng, result.receiver.lat);
          var toMarker = new BMap.Marker(toPoint, {icon: storeIcon});
          var toLabel = new BMap.Label('<div class="ware_detal"><p>门店：'+ result.receiver.name +'</p></div>', { offset: new BMap.Size(34,-10) });
          ...
          result.warehouse.forEach(item => {
            ...
            zLabel = new BMap.Label('<div class="ware_detal"><p>'+ fromName + item.orgName +'</p></div>', { offset: new BMap.Size(34,-10) });
      ...
      // 轨迹
      utilFos('/OrderMapServlet?action=getTrackNo', {
        params : {
          orderUnikey : this.unikey
        },
      }).then(response => {
        if(response.status == 200){
          var trackNo = response.data
          if(!!trackNo){
            utilFos('/OrderMapServlet?action=getRiderLocation', {
              params : {
                trackNo : trackNo,
              ...
              if(res.status == 200){
                var route = res.data.routeList["0"].point;
      ...
      // 订单节点
      utilFos('/OrderMapServlet?action=findOrderTrack', {
        params: {
        ...
        if(response.status == 200){
          this.nodeList = JSON.parse(response.data.data)
          // console.log('订单节点', this.nodeList)
        }
      })

      // 订单详情
      utilFos('/OrderMapServlet?action=showOrderInfo', {
        params: {
          orderUnikey : this.unikey
        }
      }).then(response => {
        ...
        if(response.status == 200){
          this.orderDetail = Object.assign({}, response.data.order)
        }
      })
